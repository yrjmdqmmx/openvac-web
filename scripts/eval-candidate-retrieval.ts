...
} from "../src/evals/candidate-retrieval";
...
if (!live) {
  console.log(
    JSON.stringify(
      {
        dataset: "openvac-cern-candidate-retrieval-v1",
        cases: CANDIDATE_RETRIEVAL_EVAL_CASES.length,
        uniqueIds: new Set(
          CANDIDATE_RETRIEVAL_EVAL_CASES.map((item) => item.id)
        ).size,
        mode: "structure-only",
        note: "Live evaluation is intentionally blocked until human review, embedding, and publication."
      },
      null,
      2
    )
  ...
  try {
    ...
    const results = await evaluateCandidateRetrieval((query) =>
